...
const AddStage = (props) => {
	...
	const createStage = async () => {
		...
		if(fileUrl != "" && name != "" && date != "" && description != "") {
			...
			await fetch(`http://localhost:3000/api/stage/add`, {
				method: "POST",
	...
	return (
		...
		<button className = "button-create-stage"
		onClick = { createStage }> CREATE STAGE
		</button>
		</div>
		</div>
		</div>
...
const EditStage = (props) => {
	...
	return (
		...
		<div class = "form-edit-group-stage">
		<input value = {
			(globalStages.find(element => element.name == props.stage.name)).timestamp } type = "date"
		class = "form-control-date"
		id = "end_date"
		onChange = {
			(e) => setDate(e.target.value) }/>
		...
		<button className = "button-edit-stage"
		onClick = { editStage }> EDIT STAGE
		</button>
		<button className = "button-edit-stage"
		onClick = {
			() => {
	...
	if(authorID == getIDCookie()) {
		...
		return (
			<> {
				...
				<img id = ""
				src = {
					(globalStages.find(element => element.name == props.stage.name)).image_path }/>
				...
				<EditStage stage = { props.stage } journey = { props.journey }
					setJourney = { props.setJourney } setEdit = { setEdit }/>
			}
			</>
		);
	}
